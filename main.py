...
def print_menu():
    ...
    print(f"  {Colors.BRIGHT_YELLOW}4{Colors.RESET}. {Colors.BRIGHT_WHITE}Sentence Maintenance{Colors.RESET}")
    print(f"     {Colors.DIM}View, copy, and move sentences between projects and headings{Colors.RESET}")
    print(f"     {Colors.DIM}Best for: Reorganizing and reusing content{Colors.RESET}\n")
...
def launch_sentence_maintenance():
    """Launch the sentence maintenance tool"""
    try:
        subprocess.run([sys.executable, "sentence_maintenance.py"])
    except FileNotFoundError:
        print(f"\n{Colors.RED}Error:{Colors.RESET} sentence_maintenance.py not found in current directory")
    except Exception as e:
        print(f"\n{Colors.RED}Error:{Colors.RESET} {e}")
...
def main():
    ...
    while True:
        ...
        choice = input(f"\n{Colors.BRIGHT_GREEN}Select an option (1-4, Q):{Colors.RESET} ").strip().upper()
        ...
        if choice == '1':
            ...
            clear_screen()
            launch_sentence_maintenance()
        elif choice == 'Q':
            clear_screen()
            print(f"\n{Colors.BRIGHT_CYAN}Thank you for using Project Outline System!{Colors.RESET}\n")
            break
        else:
            print(f"\n{Colors.RED}Invalid choice.{Colors.RESET} Please select 1, 2, 3, 4, or Q.")
